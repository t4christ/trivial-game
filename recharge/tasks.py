...
def recharge_session(player,difficulty,login_payload,url,phone_number,amount):
    try:
        with requests.Session() as session:
            ...
            if recharge:
                        ...
                        if get_data['status'] == 201:
                            ERCTransaction.objects.create(target=player,difficulty=difficulty,status=get_data['status'],
...
def test_recharge(player,difficulty,phone_number,amount):
            ...
            print("Test Recharge username",player)
            return recharge_session(player,difficulty,login_payload,url,phone_number,amount)
...
def remove_airtime():
    ...
    if time(9,10) <= now.time() <= time(9,15) or time(12,10) <= now.time() <= time(12,15) or time(15,10) <= now.time() <= time(15,15) or time(18,10) <= now.time() <= time(18,15) or time(21,10) <= now.time() <= time(21,15):
            if recharge_score:
                print("Recharge Score",recharge_score)
    ...
    # if winner_arr:
        ...
        if time(5,59) <= now.time() <= time(21,5)  and weekday in current_day:
                        ...
                        if airtime_score_levone.count() >= 1:
                            ...
                            high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                            if high_score == 0:
                               test_recharge(user_num.username,'levelone',airtime_score_levone[0].phone_number,100)
                        ...
                        if airtime_score_levthree.count() >= 1:
                                user_num=MyUser.objects.get(username=airtime_score_levthree[0].player)
                                high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                                if high_score == 0:
                        ...
                        if airtime_score_levfour.count() >= 1:
                                user_num=MyUser.objects.get(username=recharge_levfour_user[0].player)
                                high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                                if high_score == 0:
    ...
    # if winner_arr:
        ...
        if time(5,59) <= now.time() <= time(21,5) and weekday in current_day:
            ...
            if airtime_score_akwa.count() >= 1:
                user_num=MyUser.objects.get(username=airtime_score_akwa[0].user.username)
                print("Check for usernanme",user_num.username,airtime_score_akwa[0].user.username)
                high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                print("Username recharge count",high_score)
                if high_score == 0:
            ...
            if airtime_score_nanniv.count() >= 1:
                ...
                user_num=MyUser.objects.get(username=airtime_score_nanniv[0].user.username)
                high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                print("High Score",high_score,user_num.phone_number[-10:])
                if high_score == 0:
                        test_recharge(user_num.username,'nanniv',user_num.phone_number,100)

                else:
                    print("Anniversary winner can only win once  in 5 days",airtime_score_nanniv[0].user.username)

            # else:
            #     print('No winner for anniversary')
            if airtime_score_xmas.count() >= 1:
                            user_num=MyUser.objects.get(username=airtime_score_xmas[0].user.username)
                            high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                            if high_score == 0:
            ...
            if airtime_score_levfive.count() >= 1:
                            user_num=MyUser.objects.get(username=airtime_score_levfive[0].user.username)
                            high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                            if high_score == 0:
            ...
            if  airtime_score_easy.count() >= 1:
                                user_num=MyUser.objects.get(username=airtime_score_easy[0].user.username)
                                high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                                if high_score == 0:
            ...
            if airtime_score_medium.count() >= 1:
                                user_num=MyUser.objects.get(username=airtime_score_medium[0].user.username)
                                high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
            ...
            if airtime_score_hard.count() >= 1:
                                user_num=MyUser.objects.get(username=airtime_score_hard[0].user.username)
                                high_score=ERCTransaction.objects.filter(time__date=date.today(),target=user_num.username,phone_number__endswith=user_num.phone_number[-10:]).count()
                                if high_score == 0:
